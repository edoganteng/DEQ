...
UniValue deqync(const UniValue& params, bool fHelp)
{
    std::string strMode;
    if (params.size() == 1)
    ...
    if (fHelp || params.size() != 1 || (strMode != "status" && strMode != "reset")) {
        throw runtime_error(
            ...
            "\nExamples:\n" +
            HelpExampleCli("deqync", "\"status\"") + HelpExampleRpc("deqync", "\"status\""));
    }
...
UniValue getstakingstatus(const UniValue& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "  \"deqync\": true|false,             (boolean) if masternode data is synced\n"
            "  \"staking status\": true|false,     (boolean) if the wallet is staking or not\n"
            "}\n"
    ...
    obj.push_back(Pair("deqync", masternodeSync.IsSynced()));
