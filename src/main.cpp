...
int64_t GetBlockValue(int nHeight)
{
	...
	else if (nHeight <= 73281920 && nHeight > 750000) {
		nSubsidy = 5 * COIN;
	}
	else {
		nSubsidy = 0 * COIN;
...
int64_t GetMasternodePayment (int nHeight, unsigned int mnLevel, int64_t blockValue, int nMasternodeCount, bool isZDEQStake) {
  return GetMasternodePayment (nHeight, blockValue, nMasternodeCount, isZDEQStake) * Params ().getMasternodePhaseWeight (mnLevel, nHeight) / Params ().getMasternodePhaseWeight (0, nHeight);
}
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
	...
	//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zDEQSpent: %s\n",
...
bool ContextualCheckZerocoinStake(int nHeight, CStakeInput* stake)
{
	...
	if (CZDEQStake* zdeq = dynamic_cast<CZDEQStake*>(stake)) {
		...
		if (chainActive.Height() - pindexFrom->nHeight < Params().Zerocoin_RequiredStakeDepth())
			return error("%s: zdeq stake does not have required confirmation depth. Current height %d,  stakeInput height %d.", __func__, chainActive.Height(), pindexFrom->nHeight);
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
	...
	if (isPoS) {
		...
		// Inputs
		std::vector<CTxIn> DEQInputs;
		std::vector<CTxIn> zdeqInputs;

		for (CTxIn stakeIn : stakeTxIn.vin) {
			if (stakeIn.scriptSig.IsZerocoinSpend()) {
				zdeqInputs.push_back(stakeIn);
			}
			else {
				DEQInputs.push_back(stakeIn);
			}
		}
		const bool hasDEQInputs = !DEQInputs.empty();
		const bool hasZDEQInputs = !zdeqInputs.empty();
		...
		for (CTransaction tx : block.vtx) {
			for (CTxIn in : tx.vin) {
				...
				if (hasDEQInputs)
					...
					for (CTxIn DEQIn : DEQInputs) {
						if (DEQIn.prevout == in.prevout) {
		...
		if (isBlockFromFork) {
			...
			// Now that this loop if completed. Check if we have zdeq inputs.
			if (hasZDEQInputs) {
				for (CTxIn zDEQInput : zdeqInputs) {
					CoinSpend spend = TxInToZerocoinSpend(zDEQInput);
					...
					if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, spend, pindex, 0))
						return state.DoS(100, error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
							stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-zdeq");
...
int ActiveProtocol()
{
	...
	// SPORK_15 is used for 90003 (v3.0.0+)
