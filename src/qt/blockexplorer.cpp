...
static std::string makeHTMLTable(const std::string* pCells, int nRows, int nColudeq)
{
    std::string Table = table;
    for (int i = 0; i < nRows; i++)
        Table += makeHTMLTableRow(pCells + i * nColudeq, nColudeq);
    Table += "</table>";
    return Table;
}
...
void BlockExplorer::showEvent(QShowEvent*)
{
    if (m_NeverShown) {
        ...
        if (!GetBoolArg("-txindex", true)) {
            ...
            QMessageBox::warning(this, "Dequant Blockchain Explorer", Warning, QMessageBox::Ok);
        }
    }
}
