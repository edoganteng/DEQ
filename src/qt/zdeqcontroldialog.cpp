...
std::set<std::string> ZDEQControlDialog::setSelectedMints;
std::set<CMintMeta> ZDEQControlDialog::setMints;

ZDEQControlDialog::ZDEQControlDialog(QWidget *parent) :
    QDialog(parent, Qt::WindowSystemMenuHint | Qt::WindowTitleHint | Qt::WindowCloseButtonHint),
    ui(new Ui::ZDEQControlDialog),
    model(0)
{
    ui->setupUi(this);
    setMints.clear();
...
ZDEQControlDialog::~ZDEQControlDialog()
{
    delete ui;
}

void ZDEQControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDEQControlDialog::updateList()
{
...
void ZDEQControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDEQControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CMintMeta& mint : setMints) {
    ...
    ui->labelZDEQ_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(setSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZDEQControlLabels(nAmount, setSelectedMints.size());
}

std::vector<CMintMeta> ZDEQControlDialog::GetSelectedMints()
{
...
void ZDEQControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
