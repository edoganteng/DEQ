...
void PrivacyDialog::on_pushButtonZDEQControl_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;

    ZDEQControlDialog* zDEQControl = new ZDEQControlDialog(this);
    zDEQControl->setModel(walletModel);
    zDEQControl->exec();
}

void PrivacyDialog::setZDEQControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzDEQSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzdeq()
{
    ...
    if (!ZDEQControlDialog::setSelectedMints.empty()) {
        vMintsToFetch = ZDEQControlDialog::GetSelectedMints();
    ...
    ZDEQControlDialog::setSelectedMints.clear();
    ui->labelzDEQSelected_int->setText(QString("0"));
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " DEQ, ";
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->labelzAvailableAmount_4->setText(QString::number(zerocoinBalance/COIN) + QString(" zdeq "));
    ...
    ui->labelZsupplyAmount_2->setText(QString::number(chainActive.Tip()->GetZerocoinSupply()/COIN) + QString(" <b>zdeq </b> "));
